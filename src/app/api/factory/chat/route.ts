...
export async function POST(request: NextRequest) {
  ...
  const { message = '', mode = 'factory' } = body;

  // Forward to backend GET /chat/smart (SSE stream) with mode=factory
  headers['Accept'] = 'text/event-stream';

  const params = new URLSearchParams({ message, mode });
  if (body.conversationId) params.set('conversationId', body.conversationId);
  const targetUrl = `${BACKEND_API_URL}/chat/smart?${params.toString()}`;
...
export async function GET(request: NextRequest) {
  ...
  const targetUrl = `${BACKEND_API_URL}/chat/smart/history${url.search}`;
